...
// JAMLEE: ptr（int,uint的大小） 和 reg （通用寄存器的大小）的在 amd64 上是 8 字节。
var Widthptr int
...
// JAMLEE: 生成符号 generate DWARF symbols
var flagDWARF bool
...
// JAMLEE: 基于 LinkArch 定义自己的 Arch。这个在 asm 中也有这个做法。
type Arch struct {
...
// JAMLEE: 定义与 obj.Link （也就是常见的ctxt）相关连的 Arch。
var thearch Arch
