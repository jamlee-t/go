...
func Flushplist(ctxt *Link, plist *Plist, newprog ProgAlloc, myimportpath string) {
	...
	// JAMLEE: 下面这句 p = plink 把 p 设置为了空指针。但是这句似乎没有执行。因为它是 post 语句
	...
	for p := plist.Firstpc; p != nil; p = plink {
		...
		// JAMLEE: p 断掉下一个语句。下一个为暂时存为 plink
		plink = p.Link
		p.Link = nil
		...
		case ATEXT:
			...
			// JAMLEE: 只有 TEXT 伪指令会被选中
			text = append(text, s)
			etext = p // 当前的汇编语句
			curtext = s // s 是符号
			continue
		// JAMLEE: 对FUNCDATA伪指令进行操作
